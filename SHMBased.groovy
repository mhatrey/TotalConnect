...
Version: v0.4
  Chnages [July 28th, 2017]
  - Improvements in notifications. You should instantly receive Action notification, followed by a Success notification in 30 - 40 seconds.
  - ContactsBook integration. If you have ContactsBook enabled on SmartThings you can choose which contact and method to receive this notifications. Also you can optionally choose to receive an SMS.
  - Contact Books are disabled by default until SmartThings enables them for global audience. To enable them follow this guide: https://github.com/mhatrey/TotalConnect/blob/Tester/SmartThingsContactBooks.md
...
def confirmation(){
  ...
  dynamicPage(name: "confirmation", title: "Selected LocationID & DeviceID", uninstall: true, install: true) {
    ...
    if (location.contactBookEnabled){
      section("Select Contacts for notifications") {
        input("recipients", "contact", title: "Send notifications to") {
          input "phone", "phone", title: "Alert with text message (optional)",
...
def alarmStatusHandler(evt) {
    	if (evt.value == "away") {
          armAway()
          def message = "Smart Home Monitor is set to Away, TotalConnect is now performing ArmAway for your ${settings.selectLocation}"
          if (location.contactBookEnabled && recipients) {
        	...
        	armStay()
          def message = "Smart Home Monitor is set to Night, TotalConnect is now performing ArmStay for your ${settings.selectLocation}"
          if (location.contactBookEnabled && recipients) {
              log.debug "Contact Book enabled!"
              sendNotificationToContacts(message, recipients)
          }
        	...
        	disarm()
          def message = "Smart Home Monitor is set to Home, TotalConnect is now performing disarm for your ${settings.selectLocation}"
          if (location.contactBookEnabled && recipients) {
              log.debug "Contact Book enabled!"
              sendNotificationToContacts(message, recipients)
          }
        ...
        if(handler == "login") {
            ...
            switch(resultCode) {
                case "0": //Successful Command
                    log.debug "case 0 was executed with $resultData"
                    ...
                    state.tokenRefresh = now() //we ran a successful command, that will keep the token alive
                      ...
                      switch(handler) {
                          ...
                          case "panel":
                              ...
                              break
                          case "refresh":
                              def waitSeconds = 30 //Instead of runIn 60 seconds, lets see if we can get the notification in 30 seconds.
...
def updateAlarmStatus(alarmCode) {
    if(state.alarmCode != alarmCode) {
        if (alarmCode == "10200") {
            ...
            def message = "TotalConnect has successfully Disarmed your ${settings.selectLocation}"
            if (location.contactBookEnabled && recipients) {
                log.debug "Contact Book enabled!"
                sendNotificationToContacts(message, recipients)
            }
            ...
            def message = "TotalConnect has successfully Armed Stay your ${settings.selectLocation}"
            if (location.contactBookEnabled && recipients) {
                log.debug "Contact Book enabled!"
                sendNotificationToContacts(message, recipients)
            }
            ...
            def message = "TotalConnect has successfully Armed Away your ${settings.selectLocation}"
            if (location.contactBookEnabled && recipients) {
                log.debug "Contact Book enabled!"
                sendNotificationToContacts(message, recipients)
            }
...
def getAlarmStatus(response) {
	...
	//state.alarmStatusRefresh = now() // Dont need this in SmartApp
  log.debug alarmCode
	return alarmCode
} //returns alarmCode
