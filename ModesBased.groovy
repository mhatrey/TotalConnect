...
def checkMode(evt) {
    	if (evt.value == "Away") {
          armAway()
          def message = "SmartThings mode is set to Away, TotalConnect is now performing ArmAway for your ${settings.selectLocation}"
          if (location.contactBookEnabled && recipients) {
        	...
        	armStay()
          def message = "SmartThings mode is set to Night, TotalConnect is now performing ArmStay for your ${settings.selectLocation}"
          if (location.contactBookEnabled && recipients) {
              log.debug "Contact Book enabled!"
              sendNotificationToContacts(message, recipients)
          }
        	...
        	disarm()
          def message = "SmartThings mode is set to Home, TotalConnect is now performing disarm for your ${settings.selectLocation}"
          if (location.contactBookEnabled && recipients) {
              log.debug "Contact Book enabled!"
              sendNotificationToContacts(message, recipients)
          }
